...
a) \ $Los conjuntos abiertos cumplen las siguientes propiedades: \\
 -\ \ \ \ $\varnothing, \R^n$ son abiertos \\
- \ \ \ \ Si $A_1,...,A_n$ abiertos $\implies  \displaystyle\bigcap_{k=1}^{n}{A_k}$ abierto. \\
-\ \ \ \ Si $\{A_{\alpha}: \alpha \in \land\} $ es una familia de abiertos $ \implies \displaystyle\bigcup_{\alpha \in \land}{A_\alpha}$ abierto. \\
$b) $ \ Un conjunto $A \subseteq \R^n $ es abierto $ \Leftrightarrow \R^n/A$ es cerrado. \\
$c) $ \ Los cerrados cumplen: \\
- \ \ \ \ $\varnothing, \R^n$ son cerrados \\
- \ \ \ \ Si $F_1,...,F_n$ cerrados $\implies  \displaystyle\bigcup_{k=1}^{n}{F_k}$ cerrado. \\
-\ \ \ \ Si $\{F_{\alpha}: \alpha \in \land\} $ es una familia de cerrados $ \implies \displaystyle\bigcap_{\alpha \in \land}{F_\alpha}$ cerrado.
\end{nprop}
