...
Sea $n \in \N, a \in \R^n, r \ge 0.$ Se definen las bolas abiertas, cerradas y esferas, respectivamente, de centro a y radio r como: $$B(a,r) = \{x \in \R^n: d(x,y) < r\} $$ $$\overline{B}(a,r) = \{x \in \R^n: d(x,y) \leq r\} $$ $$S(a,r) = \{x \in \R^n: d(x,y) = r\}$$
\end{ndef}

\begin{ndef}[Sucesión convergente en $\R^n$]
Sea $n \in \N$, y $\{x_n\}$ una sucesión convergente en $\R^n$. Diremos que $\{x_n\}$ es convergente hacia un límite $a \in \R^n$ si $$ \forall \epsilon \ge 0 \ \exists m \in \N: n \geq m \implies d(x,a) < \epsilon$$
\end{ndef}
