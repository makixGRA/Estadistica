...
Demostrar: \\
 \begin{nlist}
\item Las sucesiones de Cauchy están acotadas. \\
\item Si $\{x_n\}$ es una sucesión de Cauchy y tiene una parcial $\{x_{\sigma(n)}\}$ convergente hacia un punto $x$, entonces $\{x_n\} \rightarrow x$. \\
\item Una sucesión $\{x_n\} \in \R^n $ es de Cauchy si, y sólamente si, $\{x_n\}$ es convergente (Complitud de $R^n$).
\end{nlist}
\end{ejer}
...
\begin{ejer}
Sea $n \in \N, A \in \R^n, B \subseteq A$. Demostrar que:
\begin{nlist}
	\item Un punto $x \in B$ es interior relativo a $A$ si, y sólamente si, para toda sucesión $\xn$ de puntos de $A$ tal que $\xn \to x$ se tiene que existe $m \in \N$ de manera que $n \ge m \implies \xn \in B \ \forall n \ge m$.
	\item Un punto $x \in A$ es adherente a $B$ relativo a $A$ si, y sólamente si, existe una sucesión $ \xn \in B$ de manera que $\xn \to a$.
\end{nlist}
\end{ejer}
